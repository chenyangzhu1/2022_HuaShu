...
#
# np.savetxt("XGboost结果.csv",res,delimiter=',')

# score=cross_val_score(model,myx,myz,cv=5,n_jobs=-1,scoring='neg_mean_squared_error')
# y_test_predict=model.predict(x_test)
# rmse=np.sqrt(mean_squared_error(y_test,y_test_predict))
# print(rmse)
...
np.savetxt("随机森林预测.csv",res,delimiter=',')